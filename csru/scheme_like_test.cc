...
TEST(Scheme_Test1){
  ...
  TestCode("(define fib (lambda (x) "
             "((x.lessThan x 2).ifTrueFalse "
               ...
               "(lambda (n) 1) "
               "(lambda (n) "
                 "(1.plus (fib (x.minus x 1)) (fib (x.minus x 2))) "
               ") "
             ") "
           ") )", 
           "<Proc: {|x|"
             "((x).lessThan(x, (Numeric).parse(Numeric, \"2\"))).ifTrueFalse("
               ...
               "{|n|"
                 "((Numeric).parse(Numeric, \"1\")).plus("
                   "fib((x).minus(x, (Numeric).parse(Numeric, \"1\"))), "
                   "fib((x).minus(x, (Numeric).parse(Numeric, \"2\")))"
                 ");"
               "}"
             ");"
           "}>");
  ...
  TestCode("(define tak (lambda (x,y,z) "
           ...
           "<Proc: {|x,y,z|"
             "((x).lessThan(x, (y).plus(y, (Numeric).parse(Numeric, \"1\")))).ifTrueFalse("
               ...
               "{|n|"
                 "tak("
                   "tak((x).minus(x, (Numeric).parse(Numeric, \"1\")), y, z), "
                   "tak((y).minus(y, (Numeric).parse(Numeric, \"1\")), z, x), "
